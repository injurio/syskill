...
{
	if (!is_in_range(board, row, col) || board.cells[get_index(board, row, col)] != '*') {
    return 0;
  }
  else {
    return 1;
...
int count_live_nbrs(life_board board, int row, int col)
{
	int alive = 0;
  // North
  alive += is_alive(board, row-1, col);
  // East
  alive += is_alive(board, row, col+1);
  // South
  alive += is_alive(board, row+1, col);
  // West
  alive += is_alive(board, row, col-1);
  // NorthEast
  alive += is_alive(board, row-1, col+1);
  // SouthEast
  alive += is_alive(board, row+1, col+1);
  // SouthWest
  alive += is_alive(board, row+1, col-1);
  ...
  for (row = 0; row < current.num_rows; row++) {
    for (col = 0; col < current.num_cols; col++) {
      ...
      if (is_alive(current, row, col)) {
        if (count_live_nbrs(current, row, col) == 2 || count_live_nbrs(current, row, col) == 3) {
